...
function action_EVENT_ANY_MONSTER_DIE_3017(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 241030003, 3)
...
function action_EVENT_ANY_MONSTER_DIE_3018(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 241030003, {3014}, 1, 1, 1) then
		return -1
	end
...
function action_EVENT_ANY_MONSTER_LIVE_3022(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 1, 241030003, 9, 0, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_3023(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 1, 241030001) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_groupVariable_by_group")
	  return -1
	end
	
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 241030002, suite = 1 }) then
...
function action_EVENT_CHALLENGE_FAIL_3024(context, evt)
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 241030003, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
