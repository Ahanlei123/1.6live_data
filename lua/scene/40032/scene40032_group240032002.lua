...
triggers = {
	...
	{ config_id = 1002022, name = "CHALLENGE_FAIL_2022", event = EventType.EVENT_CHALLENGE_FAIL, source = "110113", condition = "", action = "action_EVENT_CHALLENGE_FAIL_2022", trigger_count = 0 },
	{ config_id = 1002023, name = "CHALLENGE_SUCCESS_2023", event = EventType.EVENT_CHALLENGE_SUCCESS, source = "110113", condition = "", action = "action_EVENT_CHALLENGE_SUCCESS_2023", trigger_count = 0 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_2021(context, evt)
	ScriptLib.CreateFatherChallenge(context, 110113, 110113, 90, {success = 2, fail = 1, fail_on_wipe=true})
	ScriptLib.AttachChildChallenge(context, 110113, 110114, 110114, {22000,0,0,0},{},{success=1,fail=1})
	ScriptLib.AttachChildChallenge(context, 110113, 110115, 110115, {16,20,0,0},{},{success=1,fail=1})
	ScriptLib.StartFatherChallenge(context, 110113)
	return 0
end
...
function action_EVENT_CHALLENGE_FAIL_2022(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240032002, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240032001, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function action_EVENT_CHALLENGE_SUCCESS_2023(context, evt)
	-- 改变指定group组240032001中， configid为1001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240032001, 1001, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240032002, suite = 3 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
