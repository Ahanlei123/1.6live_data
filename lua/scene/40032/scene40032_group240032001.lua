...
gadgets = {
	...
	{ config_id = 1005, gadget_id = 70900202, pos = { x = 533.258, y = -0.515, z = 233.208 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_SELECT_OPTION_1003(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 3, 240032002, {2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019,2020}, 99, 5, 5) then
		return -1
	end
	...
	-- 删除指定group： 240032001 ；指定config：1002；物件身上指定option：177；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240032001, 1002, 177) then
	...
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 1002, GadgetState.GearStart) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
		...
		if 0 ~= ScriptLib.KillGroupEntity(context, { group_id = 240032001, monsters = {}, gadgets = {1005} }) then
...
function action_EVENT_GADGET_CREATE_1004(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240032001, 1002, {177}) then
