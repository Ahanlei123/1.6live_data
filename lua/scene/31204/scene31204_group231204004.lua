...
-- 怪物
monsters = {
	{ config_id = 4001, monster_id = 21020501, pos = { x = 7.472, y = 0.034, z = -12.162 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 4002, monster_id = 21020501, pos = { x = -9.591, y = -0.002, z = -12.264 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_4028(context, evt)
	...
	-- 从231204001的变量TPL_TIME中取出对应值并开启挑战
	  local tpl_time = ScriptLib.GetGroupVariableValueByGroup(context, "TPL_TIME", 231204001)
	  ...
	  if 0 ~= ScriptLib.ActiveChallenge(context, 2, 198, tpl_time, 231204004, 2, 0) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge_by_remainTime")
	    return -1
	  end
...
function action_EVENT_CHALLENGE_SUCCESS_4032(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 2, 231204001) then
...
function action_EVENT_CHALLENGE_FAIL_4033(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 231204004, suite = 1 }) then
