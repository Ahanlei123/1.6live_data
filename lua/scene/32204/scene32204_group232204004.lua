...
-- 怪物
monsters = {
	{ config_id = 4001, monster_id = 22010204, pos = { x = -0.009, y = 0.034, z = -11.636 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 6101 } },
	{ config_id = 4002, monster_id = 21020201, pos = { x = 0.141, y = 0.034, z = 12.087 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1 },
...
function condition_EVENT_ANY_MONSTER_DIE_4004(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 232204004) ~= 0 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_4004(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 232204004, 3)
...
function action_EVENT_ANY_MONSTER_LIVE_4010(context, evt)
	...
	-- 从232204001的变量TPL_TIME中取出对应值并开启挑战
	  local tpl_time = ScriptLib.GetGroupVariableValueByGroup(context, "TPL_TIME", 232204001)
	  ...
	  if 0 ~= ScriptLib.ActiveChallenge(context, 2, 198, tpl_time, 232204004, 5, 0) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge_by_remainTime")
	    return -1
	  end
...
function action_EVENT_CHALLENGE_SUCCESS_4011(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 2, 232204001) then
...
function action_EVENT_CHALLENGE_FAIL_4012(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 232204004, suite = 1 }) then
