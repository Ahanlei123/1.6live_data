...
-- 装置
gadgets = {
	{ config_id = 2001, gadget_id = 70360010, pos = { x = 0.178, y = 0.076, z = -0.122 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_SELECT_OPTION_2003(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 230102004, 4001, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组230102004中， configid为4002的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 230102004, 4002, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
