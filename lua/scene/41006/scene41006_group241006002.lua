...
function action_EVENT_GADGET_CREATE_2002(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 241006002, 2001, {177}) then
...
function condition_EVENT_SELECT_OPTION_2003(context, evt)
	...
	-- 判断变量"stage"为0
	if ScriptLib.GetGroupVariableValueByGroup(context, "stage", 241006001) ~= 0 then
			return false
	end
...
function action_EVENT_SELECT_OPTION_2003(context, evt)
	-- 创建编号为1（该怪物潮的识别id)的怪物潮，创建怪物总数为19，场上怪物最少5只，最多5只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 241006003, {3001,3002,3003,3004,3021,3005,3006,3007,3008,3009,3010,3011,3012,3013,3014,3015,3016,3018,3019}, 19, 5, 5) then
		return -1
	end
	
	-- 删除指定group： 241006002 ；指定config：2001；物件身上指定option：177；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 241006002, 2001, 177) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
	
	-- 改变指定group组241006002中， configid为2001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 241006002, 2001, GadgetState.GearStop) then
...
function action_EVENT_SELECT_OPTION_2004(context, evt)
	-- 创建编号为2（该怪物潮的识别id)的怪物潮，创建怪物总数为19，场上怪物最少5只，最多5只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 2, 241006003, {3001,3002,3003,3004,3021,3005,3006,3007,3008,3009,3010,3011,3012,3013,3014,3015,3016,3018,3019}, 19, 5, 5) then
		return -1
	end
