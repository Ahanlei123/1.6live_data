...
-- 怪物
monsters = {
	{ config_id = 4001, monster_id = 20010601, pos = { x = 3.842, y = 0.034, z = -10.976 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 4002, monster_id = 20010901, pos = { x = -3.513, y = 0.034, z = -10.885 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
...
function action_EVENT_ANY_MONSTER_DIE_4006(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 232200004, 3)
...
function action_EVENT_ANY_MONSTER_LIVE_4027(context, evt)
	...
	-- 从232200001的变量TPL_TIME中取出对应值并开启挑战
	  local tpl_time = ScriptLib.GetGroupVariableValueByGroup(context, "TPL_TIME", 232200001)
	  ...
	  if 0 ~= ScriptLib.ActiveChallenge(context, 2, 198, tpl_time, 232200004, 5, 0) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge_by_remainTime")
	    return -1
	  end
...
function action_EVENT_CHALLENGE_SUCCESS_4031(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 2, 232200001) then
...
function action_EVENT_CHALLENGE_FAIL_4032(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 232200004, suite = 1 }) then
