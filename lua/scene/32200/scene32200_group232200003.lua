...
monsters = {
	...
	{ config_id = 3004, monster_id = 20010501, pos = { x = 0.181, y = 0.034, z = -8.300 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 3005, monster_id = 20010501, pos = { x = 7.657, y = 0.034, z = -7.251 }, rot = { x = 0.000, y = 312.600, z = 0.000 }, level = 1 },
	{ config_id = 3007, monster_id = 20010501, pos = { x = -9.817, y = 0.034, z = -7.214 }, rot = { x = 0.000, y = 47.614, z = 0.000 }, level = 1 },
...
function action_EVENT_ANY_MONSTER_LIVE_3031(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 197, 600, 232200003, 25, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_3035(context, evt)
	-- 将剩余时间记录在触发此challenge的group变量TPL_TIME中
	    if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "TPL_TIME", evt.param2, 232200001) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : save_challenge_remainTime")
	      return -1
	    end
	
	-- 将本组内变量名为 "stage" 的变量设置为 1
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 1, 232200001) then
	...
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 232200002, 2001, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 232200002, 2001, {176}) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_wok_options_by_configid")
		return -1
	end
	
	-- 改变指定group组232200001中， configid为1001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 232200001, 1001, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
...
function action_EVENT_CHALLENGE_FAIL_3036(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 232200003, suite = 1 }) then
