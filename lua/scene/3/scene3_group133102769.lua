...
function LF_Notify_Platform_Sink(context, index)
	...
	elseif index == 2 then
		...
		if pid ~= 0 then
			...
			p = math.random(1,x)
			ScriptLib.SetGroupVariableValue(context, "platform4", new_p[p])
			p_arr = new_p
		else
			pid = ScriptLib.GetGroupVariableValue(context, "platform1")
			pid = LF_Get_Array_Pos(p_arr, pid)
...
-- 变量
variables = {
	{ name = "next_monster1", value = 0, no_refresh = false },
	{ name = "next_monster2", value = 0, no_refresh = false },
	{ name = "summon_platform1", value = 0, no_refresh = false },
	{ name = "summon_platform2", value = 0, no_refresh = false },
	{ name = "platform1", value = 0, no_refresh = false },
	{ name = "platform2", value = 0, no_refresh = false },
	{ name = "platform3", value = 0, no_refresh = false },
	{ name = "platform4", value = 0, no_refresh = false },
	{ name = "boss_exist", value = 1, no_refresh = false },
	{ name = "stage", value = 0, no_refresh = false },
	{ name = "REGION_EID", value = 0, no_refresh = false },
	{ name = "auth_uid_p1", value = 0, no_refresh = false },
	{ name = "auth_uid_p2", value = 0, no_refresh = false },
	{ name = "is_in_replacement", value = 0, no_refresh = true }
}
...
suites = {
	{
		...
		triggers = { "GROUP_LOAD_769052", "GROUP_REFRESH_769053", "TIME_AXIS_PASS_769056", "ENTER_REGION_769059", "LEAVE_REGION_769060", "GROUP_WILL_UNLOAD_769061", "VARIABLE_CHANGE_769068" },
		rand_weight = 100
	},
	{
		-- suite_id = 2,
	...
	},
	{
		...
		gadgets = { 769055 },
		regions = { 769057 },
		triggers = { "ANY_MONSTER_DIE_769027", "GADGET_STATE_CHANGE_769028", "VARIABLE_CHANGE_769029", "GADGET_CREATE_769030", "ENTER_REGION_769057", "GADGET_CREATE_769066", "SELECT_OPTION_769067" },
		rand_weight = 100
	},
	{
		-- suite_id = 7,
...
function action_EVENT_GADGET_STATE_CHANGE_769028(context, evt)
	...
	if evt.param1 == 0 then
		ScriptLib.CancelGroupTimerEvent(context, 133102769, "summon")
		--获取此platform位置
		local stage = ScriptLib.GetGroupVariableValue(context, "stage")
		--获取波次与对应平台信息并创建怪物
	...
	elseif evt.param1 == 202 then
		ScriptLib.CancelGroupTimerEvent(context, 133102769, "sink")
