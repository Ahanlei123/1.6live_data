...
function action_EVENT_CHALLENGE_FAIL_512022(context, evt)
	...
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 512016, GadgetState.Default) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
	    ...
	    ScriptLib.KillExtraGroupSuite(context, 133008512, 4)
	
	    -- 杀死指定group内的gadget和monster,移除其它东西
	    ScriptLib.KillExtraGroupSuite(context, 133008512, 5)
	
	    -- 杀死指定group内的gadget和monster,移除其它东西
	    ScriptLib.KillExtraGroupSuite(context, 133008512, 6)
	
	    -- 杀死指定group内的gadget和monster,移除其它东西
	    ScriptLib.KillExtraGroupSuite(context, 133008512, 7)
