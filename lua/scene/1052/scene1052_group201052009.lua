...
gadgets = {
	...
	{ config_id = 9011, gadget_id = 70900201, pos = { x = -147.251, y = 105.209, z = 110.276 }, rot = { x = 0.000, y = 104.168, z = 0.000 }, level = 1, showcutscene = true }
}
...
-- 变量
variables = {
	{ name = "type", value = 0, no_refresh = false },
	{ name = "count", value = 0, no_refresh = false },
	{ name = "step", value = 0, no_refresh = true }
}
...
function action_EVENT_SELECT_OPTION_9014(context, evt)
	-- 删除指定group： 201052009 ；指定config：9001；物件身上指定option：177；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 201052009, 9001, 177) then
	...
	-- 触发镜头注目，注目位置为坐标（-136，101，121），持续时间为8秒，并且为强制注目形式，不广播其他玩家
	  ...
	  local pos_follow = {x=0, y=0, z=0}
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 201052010, suite = 1 }) then
...
function action_EVENT_VARIABLE_CHANGE_9016(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 100, 50, 45, 3, 1021, 3) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge")
		return -1
	end
	
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 201052009, 2)
...
function action_EVENT_GADGET_STATE_CHANGE_9017(context, evt)
	...
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 201052009, 3)
...
function action_EVENT_GADGET_STATE_CHANGE_9018(context, evt)
	...
	-- 添加suite4的新内容
	    ScriptLib.AddExtraGroupSuite(context, 201052009, 4)
...
function action_EVENT_GADGET_STATE_CHANGE_9019(context, evt)
	...
	-- 添加suite5的新内容
	    ScriptLib.AddExtraGroupSuite(context, 201052009, 5)
...
function action_EVENT_CHALLENGE_SUCCESS_9026(context, evt)
	...
	if 0 ~= ScriptLib.ChangeGroupVariableValue(context, "step", 1) then
	  ...
	  return -1
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 201052009, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	
	-- 改变指定group组201052015中， configid为15001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 201052015, 15001, GadgetState.Default) then
...
function action_EVENT_SELECT_OPTION_9028(context, evt)
	...
	-- 触发镜头注目，注目位置为坐标（-136，101，121），持续时间为11秒，并且为强制注目形式，不广播其他玩家
	  ...
	  local pos_follow = {x=0, y=0, z=0}
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 201052011, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function action_EVENT_GADGET_STATE_CHANGE_9032(context, evt)
	...
	-- 添加suite6的新内容
	    ScriptLib.AddExtraGroupSuite(context, 201052009, 6)
...
function action_EVENT_VARIABLE_CHANGE_9039(context, evt)
	-- 创建编号为101（该挑战的识别id),挑战内容为53的区域挑战，具体参数填写方式，见DungeonChallengeData表中的注释，所有填写的值都必须是int类型
	if 0 ~= ScriptLib.ActiveChallenge(context, 101, 53, 60, 3, 1021, 4) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge")
		return -1
	end
...
function action_EVENT_CHALLENGE_SUCCESS_9041(context, evt)
	...
	-- 改变指定group组201052015中， configid为15002的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 201052015, 15002, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
...
function action_EVENT_SELECT_OPTION_9043(context, evt)
	...
	-- 触发镜头注目，注目位置为坐标（-136，101，121），持续时间为13秒，并且为强制注目形式，不广播其他玩家
	  ...
	  local pos_follow = {x=0, y=0, z=0}
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 201052012, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function action_EVENT_VARIABLE_CHANGE_9044(context, evt)
	-- 创建编号为102（该挑战的识别id),挑战内容为54的区域挑战，具体参数填写方式，见DungeonChallengeData表中的注释，所有填写的值都必须是int类型
	if 0 ~= ScriptLib.ActiveChallenge(context, 102, 54, 75, 3, 1021, 5) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge")
		return -1
	end
...
function action_EVENT_CHALLENGE_SUCCESS_9046(context, evt)
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 201052005, 2)
	...
	-- 改变指定group组201052015中， configid为15003的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 201052015, 15003, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
...
function action_EVENT_GADGET_STATE_CHANGE_9052(context, evt)
	...
	-- 添加suite7的新内容
	    ScriptLib.AddExtraGroupSuite(context, 201052009, 7)
