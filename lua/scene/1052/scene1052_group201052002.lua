...
function action_EVENT_SELECT_OPTION_2003(context, evt)
	-- 删除指定group： 201052002 ；指定config：2001；物件身上指定option：210；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 201052002, 2001, 210) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
	
	-- 删除指定group： 201052002 ；指定config：2001；物件身上指定option：211；
	...
	-- 延迟5秒后,向groupId为：201052002的对象,请求一次调用,并将string参数："finish" 传递过去
...
function action_EVENT_TIMER_EVENT_2014(context, evt)
	...
	-- 延迟1秒后,向groupId为：201052002的对象,请求一次调用,并将string参数："tip2" 传递过去
