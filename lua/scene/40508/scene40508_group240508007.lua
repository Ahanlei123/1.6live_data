...
-- 装置
gadgets = {
	{ config_id = 7001, gadget_id = 70350023, pos = { x = -0.100, y = 0.100, z = 0.100 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_SELECT_OPTION_7003(context, evt)
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240508005, 2)
	
	-- 删除指定group： 240508007 ；指定config：7001；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240508007, 7001, 7) then
		return -1
	end
	
	-- 改变指定group组240508007中， configid为7001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240508007, 7001, GadgetState.GearStart) then
			return -1
		end 
	
	-- 改变指定group组240508004中， configid为4001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240508004, 4001, GadgetState.Default) then
			return -1
		end 
	
	-- 改变指定group组240508004中， configid为4002的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240508004, 4002, GadgetState.Default) then
			return -1
		end 
