...
function action_EVENT_GADGET_CREATE_2002(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 241000002, 2001, {177}) then
...
function action_EVENT_SELECT_OPTION_2003(context, evt)
	-- 创建编号为1（该怪物潮的识别id)的怪物潮，创建怪物总数为24，场上怪物最少6只，最多6只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 241000003, {3001,3002,3003,3004,3005,3006,3007,3008,3009,3010,3015,3021}, 24, 6, 6) then
		return -1
	end
	
	-- 删除指定group： 241000002 ；指定config：2001；物件身上指定option：177；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 241000002, 2001, 177) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
	
	-- 改变指定group组241000002中， configid为2001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 241000002, 2001, GadgetState.GearStop) then
...
function action_EVENT_SELECT_OPTION_2004(context, evt)
	-- 创建编号为2（该怪物潮的识别id)的怪物潮，创建怪物总数为25，场上怪物最少6只，最多6只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 2, 241000003, {3001,3002,3003,3004,3005,3006,3007,3008,3009,3010,3015,3021}, 25, 6, 6) then
		return -1
	end
