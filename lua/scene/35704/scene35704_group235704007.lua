...
-- 怪物
monsters = {
	{ config_id = 7001, monster_id = 23010101, pos = { x = 6.069, y = 0.012, z = -11.151 }, rot = { x = 0.000, y = 346.531, z = 0.000 }, level = 1, affix = { 6101 }, isElite = true },
	{ config_id = 7002, monster_id = 23010301, pos = { x = -8.689, y = 0.012, z = -11.392 }, rot = { x = 0.000, y = 21.465, z = 0.000 }, level = 1 },
	...
	{ config_id = 7006, monster_id = 23010501, pos = { x = -7.759, y = 0.012, z = -8.438 }, rot = { x = 0.000, y = 21.465, z = 0.000 }, level = 1 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_7010(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 197, 600, 235704007, 4, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_7011(context, evt)
	-- 将剩余时间记录在触发此challenge的group变量TPL_TIME中
	    if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "TPL_TIME", evt.param2, 235704005) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : save_challenge_remainTime")
	      return -1
	    end
	
	-- 将本组内变量名为 "stage" 的变量设置为 1
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 1, 235704005) then
	...
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 235704006, 6001, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 235704006, 6001, {176}) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_wok_options_by_configid")
		return -1
	end
	
	-- 改变指定group组235704005中， configid为5001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 235704005, 5001, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
...
function action_EVENT_CHALLENGE_FAIL_7012(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 235704007, suite = 1 }) then
...
function condition_EVENT_ANY_MONSTER_DIE_7013(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 235704007) ~= 0 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_7013(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 235704007, 3)
