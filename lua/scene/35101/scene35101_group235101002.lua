...
-- 怪物
monsters = {
	{ config_id = 2001, monster_id = 21030101, pos = { x = -4.744, y = -0.071, z = -10.049 }, rot = { x = 0.000, y = 8.978, z = 0.000 }, level = 1 },
	{ config_id = 2002, monster_id = 21030101, pos = { x = 5.053, y = -0.071, z = -9.935 }, rot = { x = 0.000, y = 341.505, z = 0.000 }, level = 1 },
	{ config_id = 2007, monster_id = 21020201, pos = { x = -0.129, y = -0.071, z = -7.812 }, rot = { x = 0.000, y = 341.505, z = 0.000 }, level = 1 },
	{ config_id = 2008, monster_id = 22010302, pos = { x = 4.825, y = -0.071, z = -15.861 }, rot = { x = 0.000, y = 349.618, z = 0.000 }, level = 1 },
	{ config_id = 2009, monster_id = 22010303, pos = { x = -3.832, y = -0.071, z = -15.553 }, rot = { x = 0.000, y = 8.978, z = 0.000 }, level = 1 }
}
...
-- 装置
gadgets = {
	{ config_id = 2003, gadget_id = 70900205, pos = { x = 6.023, y = -1.530, z = -2.204 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_CHALLENGE_FAIL_2004(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 235101002, suite = 1 }) then
...
function action_EVENT_ANY_MONSTER_LIVE_2005(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 1024, 300, 235101002, 5, 0) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge")
		return -1
	end
	
	-- 通知groupid为235101002中,configid为：2001的怪物入战或者脱战，set为1是入战，为0是脱战
	if 0 ~= ScriptLib.SetMonsterBattleByGroup(context, 2001, 235101002) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_monster_battle_by_group")
	  return -1
	end
	
	-- 通知groupid为235101002中,configid为：2002的怪物入战或者脱战，set为1是入战，为0是脱战
	if 0 ~= ScriptLib.SetMonsterBattleByGroup(context, 2002, 235101002) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_monster_battle_by_group")
	  return -1
	end
...
function action_EVENT_CHALLENGE_SUCCESS_2006(context, evt)
	-- 改变指定group组235101001中， configid为1002的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 235101001, 1002, GadgetState.GearStart) then
...
function action_EVENT_ANY_MONSTER_DIE_2011(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 235101002, 3)
