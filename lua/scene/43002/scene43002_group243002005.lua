...
gadgets = {
	...
	{ config_id = 5008, gadget_id = 70900201, pos = { x = 31.932, y = 41.574, z = -45.538 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 5009, gadget_id = 70900201, pos = { x = 32.364, y = -19.558, z = -51.899 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	...
	{ config_id = 5017, gadget_id = 70360161, pos = { x = 98.186, y = 40.000, z = -4.138 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 5018, gadget_id = 70360160, pos = { x = 74.599, y = 40.000, z = -71.325 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 5019, gadget_id = 70360162, pos = { x = -3.011, y = 40.033, z = -50.176 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
regions = {
	...
	{ config_id = 5021, shape = RegionShape.CUBIC, size = { x = 50.000, y = 10.000, z = 50.000 }, pos = { x = 32.125, y = -18.007, z = -52.260 } }
}
...
triggers = {
	...
	{ config_id = 1005021, name = "ENTER_REGION_5021", event = EventType.EVENT_ENTER_REGION, source = "", condition = "condition_EVENT_ENTER_REGION_5021", action = "", tag = "902" },
	{ config_id = 1005022, name = "GADGET_STATE_CHANGE_5022", event = EventType.EVENT_GADGET_STATE_CHANGE, source = "", condition = "condition_EVENT_GADGET_STATE_CHANGE_5022", action = "action_EVENT_GADGET_STATE_CHANGE_5022", tag = "902" },
	{ config_id = 1005023, name = "VARIABLE_CHANGE_5023", event = EventType.EVENT_VARIABLE_CHANGE, source = "success", condition = "condition_EVENT_VARIABLE_CHANGE_5023", action = "action_EVENT_VARIABLE_CHANGE_5023", tag = "903" },
	...
	{ config_id = 1005025, name = "VARIABLE_CHANGE_5025", event = EventType.EVENT_VARIABLE_CHANGE, source = "runes", condition = "condition_EVENT_VARIABLE_CHANGE_5025", action = "action_EVENT_VARIABLE_CHANGE_5025" },
	{ config_id = 1005026, name = "VARIABLE_CHANGE_5026", event = EventType.EVENT_VARIABLE_CHANGE, source = "runes", condition = "condition_EVENT_VARIABLE_CHANGE_5026", action = "action_EVENT_VARIABLE_CHANGE_5026" },
	{ config_id = 1005027, name = "CHALLENGE_FAIL_5027", event = EventType.EVENT_CHALLENGE_FAIL, source = "", condition = "condition_EVENT_CHALLENGE_FAIL_5027", action = "action_EVENT_CHALLENGE_FAIL_5027", trigger_count = 0 }
}

-- 变量
variables = {
	{ name = "runes", value = 0, no_refresh = false },
	{ name = "AddTime", value = 0, no_refresh = false },
...
suites = {
	{
		...
		regions = { 5014, 5021 },
		triggers = { "CHALLENGE_FAIL_5001", "TIME_AXIS_PASS_5002", "VARIABLE_CHANGE_5003", "SELECT_OPTION_5006", "TIMER_EVENT_5007", "GADGET_CREATE_5011", "SELECT_OPTION_5012", "ENTER_REGION_5014", "VARIABLE_CHANGE_5015", "VARIABLE_CHANGE_5020", "ENTER_REGION_5021", "GADGET_STATE_CHANGE_5022", "VARIABLE_CHANGE_5023", "VARIABLE_CHANGE_5024", "VARIABLE_CHANGE_5025", "VARIABLE_CHANGE_5026", "CHALLENGE_FAIL_5027" },
		rand_weight = 100
	},
	{
		-- suite_id = 2,
...
function action_EVENT_CHALLENGE_FAIL_5001(context, evt)
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243002004, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243002001, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243002008, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243002009, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243002010, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243002011, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243002016, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	...
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243002017, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
...
function action_EVENT_TIME_AXIS_PASS_5002(context, evt)
	-- 重新生成指定group，默认为独立随机
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243002013 }) then
...
function action_EVENT_VARIABLE_CHANGE_5003(context, evt)
	-- 删除指定group： 243002005 ；指定config：5013；物件身上指定option：30；
...
function action_EVENT_TIMER_EVENT_5007(context, evt)
		-- 卸载指定gadget
		if 0 ~= ScriptLib.RemoveEntityByConfigId(context, 243002005, EntityType.GADGET, 5016 ) then
...
function action_EVENT_SELECT_OPTION_5012(context, evt)
	...
	-- 延迟2秒后,向groupId为：243002005的对象,请求一次调用,并将string参数："floor" 传递过去
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 243002005, "floor", 2) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : create_timerevent_by_group")
	  return -1
	end
	
	-- 删除指定group： 243002005 ；指定config：5013；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 243002005, 5013, 59) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
...
function action_EVENT_ENTER_REGION_5014(context, evt)
	...
	ScriptLib.StartGallery(context, 7008)
	
	-- 使灭队event失效
	if 0 ~= ScriptLib.ChangeGroupVariableValueByGroup(context, "AvatarDie", 1, 243002015) then
...
-- 触发条件
function condition_EVENT_GADGET_STATE_CHANGE_5022(context, evt)
...
-- 触发操作
function action_EVENT_GADGET_STATE_CHANGE_5022(context, evt)
	--子挑战 打开地板
	--ScriptLib.AttachChildChallenge(context, 999, 903, 231, --{3,903,1,1},{},{success=99999,fail=1})
...
-- 触发条件
function condition_EVENT_VARIABLE_CHANGE_5026(context, evt)
	if evt.param1 == evt.param2 then return false end
...
-- 触发条件
function condition_EVENT_CHALLENGE_FAIL_5027(context, evt)
...
-- 触发操作
function action_EVENT_CHALLENGE_FAIL_5027(context, evt)
