...
-- 装置
gadgets = {
	{ config_id = 2001, gadget_id = 70360010, pos = { x = 0.000, y = 0.500, z = 0.000 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_SELECT_OPTION_2003(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240609003, suite = 2 }) then
			return -1
		end
	
	-- 删除指定group： 240609002 ；指定config：2001；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240609002, 2001, 7) then
		return -1
	end
	
	-- 改变指定group组240609002中， configid为2001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240609002, 2001, GadgetState.GearStop) then
			return -1
		end 
	
	-- 改变指定group组240609001中， configid为1002的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240609001, 1002, GadgetState.Default) then
			return -1
		end 
	
	-- 改变指定group组240609001中， configid为1001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240609001, 1001, GadgetState.Default) then
			return -1
		end 
