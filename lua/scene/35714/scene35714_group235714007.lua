...
-- 怪物
monsters = {
	{ config_id = 7001, monster_id = 20040501, pos = { x = -0.009, y = -0.044, z = -9.729 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true }
}
...
function action_EVENT_ANY_MONSTER_LIVE_7013(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 197, 600, 235714007, 1, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_7014(context, evt)
	-- 将剩余时间记录在触发此challenge的group变量TPL_TIME中
	    if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "TPL_TIME", evt.param2, 235714005) then
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 1, 235714005) then
	...
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 235714006, 6001, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 235714006, 6001, {176}) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_wok_options_by_configid")
		return -1
	end
	
	-- 改变指定group组235714005中， configid为5001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 235714005, 5001, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	...
	-- 删除suite4的所有内容
	    ScriptLib.RemoveExtraGroupSuite(context, 235714006, 4)
...
function action_EVENT_CHALLENGE_FAIL_7015(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 235714007, suite = 1 }) then
