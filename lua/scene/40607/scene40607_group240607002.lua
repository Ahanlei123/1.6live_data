...
function action_EVENT_SELECT_OPTION_2003(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240607003, suite = 2 }) then
			return -1
		end
	
	-- 删除指定group： 240607002 ；指定config：2001；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240607002, 2001, 7) then
		return -1
	end
	
	-- 改变指定group组240607002中， configid为2001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240607002, 2001, GadgetState.GearStop) then
			return -1
		end 
	
	-- 改变指定group组240607001中， configid为1002的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240607001, 1002, GadgetState.Default) then
			return -1
		end 
	
	-- 改变指定group组240607001中， configid为1001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240607001, 1001, GadgetState.Default) then
			return -1
		end 
