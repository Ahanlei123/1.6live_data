...
-- 怪物
monsters = {
	{ config_id = 8003, monster_id = 22030102, pos = { x = -0.032, y = 0.030, z = -12.121 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true, pose_id = 101 },
	{ config_id = 8004, monster_id = 22010202, pos = { x = -0.004, y = 0.030, z = 12.209 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1, disableWander = true, affix = { 6101 } }
}
...
function action_EVENT_ANY_MONSTER_LIVE_8010(context, evt)
	...
	-- 从234714005的变量TPL_TIME中取出对应值并开启挑战
	  local tpl_time = ScriptLib.GetGroupVariableValueByGroup(context, "TPL_TIME", 234714005)
	  ...
	  if 0 ~= ScriptLib.ActiveChallenge(context, 2, 198, tpl_time, 234714008, 2, 0) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge_by_remainTime")
	    return -1
	  end
...
function action_EVENT_CHALLENGE_SUCCESS_8012(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 2, 234714005) then
...
function action_EVENT_CHALLENGE_FAIL_8013(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 234714008, suite = 1 }) then
