...
-- 怪物
monsters = {
	{ config_id = 5001, monster_id = 24010301, pos = { x = 0.136, y = -0.110, z = -14.630 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 6101 }, isElite = true, pose_id = 100 },
	{ config_id = 5002, monster_id = 24010301, pos = { x = -0.001, y = -0.110, z = 13.647 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1, pose_id = 100 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_5012(context, evt)
	...
	-- 从235710002的变量TPL_TIME中取出对应值并开启挑战
	  local tpl_time = ScriptLib.GetGroupVariableValueByGroup(context, "TPL_TIME", 235710002)
	  ...
	  if 0 ~= ScriptLib.ActiveChallenge(context, 2, 198, tpl_time, 235710005, 2, 0) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge_by_remainTime")
	    return -1
	  end
...
function action_EVENT_CHALLENGE_SUCCESS_5015(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 2, 235710002) then
...
function action_EVENT_CHALLENGE_FAIL_5016(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 235710005, suite = 1 }) then
