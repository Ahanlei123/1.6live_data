...
function action_EVENT_SELECT_OPTION_1003(context, evt)
	...
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240022002, suite = 2, refresh_level_revise = monster_level, exclude_prev = true }) then
		return -1
	end
	
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240022001, 1002, GadgetState.GearStop) then
		return -1
	end
	
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240022001, 1002, 177) then
		return -1
	end
...
function action_EVENT_GADGET_CREATE_1004(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240022001, 1002, {177}) then
