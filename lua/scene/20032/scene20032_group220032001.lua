...
suites = {
	{
		...
		regions = { },
		triggers = { "ANY_MONSTER_DIE_1009", "ANY_MONSTER_DIE_1011", "ANY_MONSTER_DIE_1024", "ANY_MONSTER_DIE_1031" },
		rand_weight = 100
	}
}
...
function condition_EVENT_ANY_MONSTER_DIE_1011(context, evt)
	--判断当前场景死亡的怪物总数不少于13
	if ScriptLib.GetMonsterDieCount(context) ~= 13 then
		return false
	end
...
function condition_EVENT_ANY_MONSTER_DIE_1024(context, evt)
	--判断当前场景死亡的怪物总数不少于5
	if ScriptLib.GetMonsterDieCount(context) ~= 5 then
		return false
	end
...
function condition_EVENT_ANY_MONSTER_DIE_1031(context, evt)
	--判断当前场景死亡的怪物总数不少于8
	if ScriptLib.GetMonsterDieCount(context) ~= 8 then
		return false
	end
