...
-- 怪物
monsters = {
	{ config_id = 5001, monster_id = 26040101, pos = { x = -5.044, y = -0.102, z = -12.197 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true },
	{ config_id = 5002, monster_id = 26040103, pos = { x = 4.117, y = -0.102, z = -12.171 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true }
}
...
function action_EVENT_ANY_MONSTER_LIVE_5012(context, evt)
	...
	-- 从233716002的变量TPL_TIME中取出对应值并开启挑战
	  local tpl_time = ScriptLib.GetGroupVariableValueByGroup(context, "TPL_TIME", 233716002)
	  ...
	  if 0 ~= ScriptLib.ActiveChallenge(context, 2, 198, tpl_time, 233716005, 2, 0) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge_by_remainTime")
	    return -1
	  end
...
function action_EVENT_CHALLENGE_SUCCESS_5015(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 2, 233716002) then
...
function action_EVENT_CHALLENGE_FAIL_5016(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 233716005, suite = 1 }) then
