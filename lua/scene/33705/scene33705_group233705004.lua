...
-- 怪物
monsters = {
	{ config_id = 4001, monster_id = 21020202, pos = { x = -0.004, y = 0.046, z = 12.183 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1, affix = { 6105 }, isElite = true },
	{ config_id = 4014, monster_id = 22010102, pos = { x = 0.053, y = 0.034, z = -9.211 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 4020, monster_id = 22010104, pos = { x = -2.303, y = 0.034, z = -9.892 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 6105 }, isElite = true },
...
function action_EVENT_ANY_MONSTER_LIVE_4028(context, evt)
	...
	-- 从233705001的变量TPL_TIME中取出对应值并开启挑战
	  local tpl_time = ScriptLib.GetGroupVariableValueByGroup(context, "TPL_TIME", 233705001)
	  ...
	  if 0 ~= ScriptLib.ActiveChallenge(context, 2, 198, tpl_time, 233705004, 4, 0) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge_by_remainTime")
	    return -1
	  end
...
function condition_EVENT_ANY_MONSTER_DIE_4029(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 233705004) ~= 0 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_4029(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 233705004, 3)
...
function action_EVENT_CHALLENGE_SUCCESS_4032(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 2, 233705001) then
...
function action_EVENT_CHALLENGE_FAIL_4033(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 233705004, suite = 1 }) then
