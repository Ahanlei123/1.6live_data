...
-- 装置
gadgets = {
	{ config_id = 2001, gadget_id = 70360010, pos = { x = 0.127, y = 0.110, z = -0.046 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_GADGET_CREATE_2002(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 241009002, 2001, {177}) then
...
function condition_EVENT_SELECT_OPTION_2003(context, evt)
	...
	-- 判断变量"stage"为0
	if ScriptLib.GetGroupVariableValueByGroup(context, "stage", 241009001) ~= 0 then
			return false
	end
...
function action_EVENT_SELECT_OPTION_2003(context, evt)
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 241009003, 2)
	
	-- 删除指定group： 241009002 ；指定config：2001；物件身上指定option：177；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 241009002, 2001, 177) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
	
	-- 改变指定group组241009002中， configid为2001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 241009002, 2001, GadgetState.GearStop) then
