...
-- 怪物
monsters = {
	{ config_id = 3004, monster_id = 22010202, pos = { x = -4.561, y = 0.080, z = -14.810 }, rot = { x = 0.000, y = 8.978, z = 0.000 }, level = 1 },
	{ config_id = 3005, monster_id = 20010901, pos = { x = 4.699, y = 0.080, z = -16.256 }, rot = { x = 0.000, y = 341.505, z = 0.000 }, level = 1 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_3031(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 1004, 300, 250024003, 2, 0) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge")
		return -1
	end
	
	-- 通知groupid为250024003中,configid为：3004的怪物入战或者脱战，set为1是入战，为0是脱战
	if 0 ~= ScriptLib.SetMonsterBattleByGroup(context, 3004, 250024003) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_monster_battle_by_group")
	  return -1
	end
	
	-- 通知groupid为250024003中,configid为：3005的怪物入战或者脱战，set为1是入战，为0是脱战
	if 0 ~= ScriptLib.SetMonsterBattleByGroup(context, 3005, 250024003) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_monster_battle_by_group")
	  return -1
	end
...
function action_EVENT_CHALLENGE_SUCCESS_3035(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 250024001, 1002, GadgetState.GearStart) then
