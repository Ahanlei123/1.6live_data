...
function action_EVENT_ANY_MONSTER_LIVE_4022(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 1, 241026004, 15, 0, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_4023(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 1, 241026006) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_groupVariable_by_group")
	  return -1
	end
	
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 241026005, suite = 1 }) then
...
function action_EVENT_CHALLENGE_FAIL_4024(context, evt)
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 241026004, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
