...
function action_EVENT_GADGET_CREATE_5002(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 241026005, 5001, {177}) then
...
function condition_EVENT_SELECT_OPTION_5003(context, evt)
	...
	-- 判断变量"stage"为0
	if ScriptLib.GetGroupVariableValueByGroup(context, "stage", 241026006) ~= 0 then
			return false
	end
...
function action_EVENT_SELECT_OPTION_5003(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 241026004, {4001,4002,4003,4004,4005,4006,4007,4008,4009,4010,4011,4012,4013,4014,4015}, 15, 5, 5) then
		return -1
	end
	
	-- 删除指定group： 241026005 ；指定config：5001；物件身上指定option：177；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 241026005, 5001, 177) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
	
	-- 改变指定group组241026005中， configid为5001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 241026005, 5001, GadgetState.GearStop) then
...
function condition_EVENT_SELECT_OPTION_5004(context, evt)
	...
	-- 判断变量"stage"为1
	if ScriptLib.GetGroupVariableValueByGroup(context, "stage", 241026006) ~= 1 then
			return false
	end
...
function action_EVENT_SELECT_OPTION_5004(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 2, 241026004, {4001,4002,4003,4004,4005,4006,4007,4008,4009,4010,4011,4012,4013,4014,4015}, 15, 5, 5) then
		return -1
	end
