...
-- 装置
gadgets = {
	{ config_id = 1001, gadget_id = 70360010, pos = { x = 0.000, y = 0.130, z = 0.000 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
...
function action_EVENT_GADGET_CREATE_1003(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240024001, 1001, {177}) then
...
function action_EVENT_SELECT_OPTION_1004(context, evt)
	...
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240024002, suite = 2, refresh_level_revise = monster_level, exclude_prev = true }) then
		return -1
	end
	
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240024001, 1001, GadgetState.GearStop) then
		return -1
	end
	
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240024001, 1001, 177) then
		return -1
	end
