...
-- 装置
gadgets = {
	{ config_id = 3001, gadget_id = 70900205, pos = { x = 2.071, y = -1.691, z = 9.991 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_18(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 2, 163, 180, 240203003, 11, 0) then
	...
	-- 删除指定group： 240203002 ；指定config：1；物件身上指定option：12；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240203002, 1, 12) then
...
function action_EVENT_CHALLENGE_FAIL_19(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240203002, 1, {12}) then
	...
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240203002, 1, GadgetState.Default) then
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240203003, suite = 2 }) then
