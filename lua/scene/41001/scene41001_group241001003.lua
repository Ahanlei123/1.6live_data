...
function condition_EVENT_ANY_MONSTER_DIE_3022(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 241001003) ~= 0 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_3022(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 241001002, suite = 1 }) then
...
function action_EVENT_ANY_MONSTER_LIVE_3031(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 1, 241001003, 15, 0, 0) then
...
function action_EVENT_ANY_MONSTER_DIE_3032(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 241001003, 3)
...
function action_EVENT_ANY_MONSTER_DIE_3033(context, evt)
	-- 添加suite4的新内容
	    ScriptLib.AddExtraGroupSuite(context, 241001003, 4)
...
function action_EVENT_CHALLENGE_SUCCESS_3035(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 1, 241001001) then
...
function action_EVENT_CHALLENGE_FAIL_3036(context, evt)
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 241001003, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function condition_EVENT_ANY_MONSTER_DIE_3037(context, evt)
	...
	-- 判断变量"stage"为1
	if ScriptLib.GetGroupVariableValueByGroup(context, "stage", 241001001) ~= 1 then
			return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_3037(context, evt)
	-- 添加suite6的新内容
	    ScriptLib.AddExtraGroupSuite(context, 241001003, 6)
