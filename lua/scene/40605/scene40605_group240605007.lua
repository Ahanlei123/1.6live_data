...
-- 装置
gadgets = {
	{ config_id = 7001, gadget_id = 70350023, pos = { x = 0.046, y = 0.120, z = 0.042 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_SELECT_OPTION_7003(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 240605005, {5004,5006,5010,5007,5008,5011,5009,5012,5001,5014,5016,5003,5018,5020,5013,5029,5030,5042,5035,5040,5043,5041,5044,5027,5046,5047,5028,5048,5049,5045}, 30, 5, 5) then
		return -1
	end
	
	-- 删除指定group： 240605007 ；指定config：7001；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240605007, 7001, 7) then
		return -1
	end
	
	-- 改变指定group组240605007中， configid为7001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240605007, 7001, GadgetState.GearStart) then
			return -1
		end 
	
	-- 改变指定group组240605004中， configid为4002的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240605004, 4002, GadgetState.Default) then
			return -1
		end 
	
	-- 改变指定group组240605004中， configid为4001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240605004, 4001, GadgetState.Default) then
			return -1
		end 
