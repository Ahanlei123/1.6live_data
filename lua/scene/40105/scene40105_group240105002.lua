...
function action_EVENT_CHALLENGE_FAIL_8(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240105001, 1001, {12}) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_wok_options_by_configid")
		return -1
	end
	
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240105002, suite = 1 }) then
...
function action_EVENT_CHALLENGE_SUCCESS_19(context, evt)
	-- 杀死Group内所有monster
		if 0 ~= ScriptLib.KillGroupEntity(context, { group_id = 240105002, kill_policy = GroupKillPolicy.GROUP_KILL_MONSTER }) then
