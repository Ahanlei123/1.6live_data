...
-- 区域
regions = {
	{ config_id = 17001, shape = RegionShape.SPHERE, radius = 16.4, pos = { x = -53.844, y = -67.648, z = -105.014 } }
}
...
function action_EVENT_SELECT_OPTION_33(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 220027003, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	
	-- 改变指定group组220027003中， configid为3010的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220027003, 3010, GadgetState.Default) then
