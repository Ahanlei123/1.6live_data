...
-- 怪物
monsters = {
	{ config_id = 25, monster_id = 20040101, pos = { x = 134.223, y = -86.953, z = -200.906 }, rot = { x = 0.000, y = 271.536, z = 0.000 }, level = 1 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_17(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 2, 182, 480, 220027016, 1, 0) then
...
function action_EVENT_CHALLENGE_FAIL_18(context, evt)
	-- 改变指定group组220027010中， configid为107的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220027010, 107, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 220027010, 107, {7}) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_wok_options_by_configid")
		return -1
	end
	
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 220027016, suite = 2 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	
	-- 改变指定group组220027010中， configid为74的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220027010, 74, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
...
function action_EVENT_ANY_MONSTER_DIE_51(context, evt)
	-- 延迟2秒后,向groupId为：220027019的对象,请求一次调用,并将string参数："Boss" 传递过去
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 220027019, "Boss", 2) then
