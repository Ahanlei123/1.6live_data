...
function action_EVENT_MONSTER_BATTLE_48(context, evt)
	-- 延迟1秒后,向groupId为：220027019的对象,请求一次调用,并将string参数："First" 传递过去
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 220027019, "First", 1) then
...
function condition_EVENT_ANY_MONSTER_DIE_49(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 220027020) ~= 0 then
		return false
	end
