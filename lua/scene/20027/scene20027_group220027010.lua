...
function action_EVENT_SELECT_OPTION_16(context, evt)
	...
	if 0 ~= ScriptLib.SetGadgetStateByConfigId(context, 107, GadgetState.GearStop) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_configId")
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 220027016, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	
	-- 删除指定group： 220027010 ；指定config：107；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 220027010, 107, 7) then
	...
	-- 改变指定group组220027010中， configid为74的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220027010, 74, GadgetState.Default) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组220027019中， configid为114的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220027019, 114, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组220027019中， configid为117的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220027019, 117, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组220027019中， configid为119的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220027019, 119, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组220027019中， configid为120的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220027019, 120, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组220027019中， configid为121的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220027019, 121, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
	-- 改变指定group组220027019中， configid为122的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 220027019, 122, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
