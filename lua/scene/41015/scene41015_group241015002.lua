...
function action_EVENT_GADGET_CREATE_2002(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 241015002, 2001, {177}) then
...
function condition_EVENT_SELECT_OPTION_2003(context, evt)
	...
	-- 判断变量"stage"为0
	if ScriptLib.GetGroupVariableValueByGroup(context, "stage", 241015001) ~= 0 then
			return false
	end
...
function action_EVENT_SELECT_OPTION_2003(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 241015003, {3001,3002,3003,3004,3005,3006,3007,3008,3009,3010,3011,3012,3013,3014}, 14, 5, 5) then
		return -1
	end
	
	-- 删除指定group： 241015002 ；指定config：2001；物件身上指定option：177；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 241015002, 2001, 177) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
	
	-- 改变指定group组241015002中， configid为2001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 241015002, 2001, GadgetState.GearStop) then
...
function action_EVENT_SELECT_OPTION_2004(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 2, 241015003, {3001,3002,3003,3004,3005,3006,3007,3008,3009,3010,3011,3012,3013,3014}, 14, 5, 5) then
		return -1
	end
