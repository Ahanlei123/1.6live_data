...
-- 装置
gadgets = {
	{ config_id = 1001, gadget_id = 70360010, pos = { x = 0.000, y = 0.012, z = 0.000 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 1010, gadget_id = 44000006, pos = { x = 0.000, y = 0.000, z = 14.000 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 1012, gadget_id = 70950042, pos = { x = 0.000, y = 0.193, z = 0.000 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
-- 点位
points = {
	{ config_id = 1002, pos = { x = 0.020, y = -0.102, z = -10.138 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, tag = 2 }
}
...
function condition_EVENT_GADGET_STATE_CHANGE_1003(context, evt)
	if 1001 ~= evt.param2 or GadgetState.GearStop ~= evt.param1 then
		return false
	end
...
function action_EVENT_GADGET_STATE_CHANGE_1003(context, evt)
	...
	-- 创建编号为201的子挑战：杀怪挑战
	if 0 ~= ScriptLib.AttachChildChallenge(context, 1, 201, 110190, {240038001,1},{},{success=0,fail=0}) then
		return -1
	end
	...
	if 0 ~= challenge_time then
		ScriptLib.AttachChildChallenge(context, 1, 202, 110191, {challenge_time,240038001,1},{},{success=0,fail=0})
	else
		ScriptLib.AddExtraGroupSuite(context, 240038001, 3)
	end
	...
	--  刷怪
	ScriptLib.CreateEffigyChallengeMonster(context, 240038001, {15008})
	
	-- 卸载回血gadget
	if 0 ~= ScriptLib.RemoveEntityByConfigId(context, 240038001, EntityType.GADGET, 1010 ) then
...
function action_EVENT_CHALLENGE_FAIL_1005(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240038001, suite = 1 }) then
...
function action_EVENT_GADGET_CREATE_1013(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 240038001, 1001, {175}) then
...
function action_EVENT_SELECT_OPTION_1014(context, evt)
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 240038001, 2)
	
	-- 删除指定group： 240038001 ；指定config：1001；物件身上指定option：175；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 240038001, 1001, 175) then
