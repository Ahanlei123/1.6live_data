...
-- 装置
gadgets = {
	{ config_id = 111, gadget_id = 70360010, pos = { x = -56.259, y = -67.443, z = -160.439 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}

-- 区域
regions = {
	{ config_id = 17001, shape = RegionShape.SPHERE, radius = 14.2, pos = { x = -55.101, y = -64.673, z = -105.756 } }
}
...
triggers = {
	...
	{ config_id = 1017001, name = "ENTER_REGION_17001", event = EventType.EVENT_ENTER_REGION, source = "", condition = "condition_EVENT_ENTER_REGION_17001", action = "action_EVENT_ENTER_REGION_17001", trigger_count = 0 }
}
...
function action_EVENT_SELECT_OPTION_33(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 201024003, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	
	-- 改变指定group组201024003中， configid为3010的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 201024003, 3010, GadgetState.Default) then
