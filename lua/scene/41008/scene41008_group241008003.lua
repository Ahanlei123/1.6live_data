...
-- 怪物
monsters = {
	{ config_id = 3001, monster_id = 21011001, pos = { x = 1.422, y = 0.019, z = -10.953 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true },
	{ config_id = 3002, monster_id = 21011001, pos = { x = -1.241, y = 0.020, z = -10.918 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true },
	{ config_id = 3003, monster_id = 21010201, pos = { x = 4.448, y = 0.021, z = -10.149 }, rot = { x = 0.000, y = 330.000, z = 0.000 }, level = 1, disableWander = true },
	{ config_id = 3004, monster_id = 21011201, pos = { x = -4.118, y = 0.021, z = -10.174 }, rot = { x = 0.000, y = 30.000, z = 0.000 }, level = 1, disableWander = true },
	{ config_id = 3006, monster_id = 21020201, pos = { x = 0.078, y = -0.038, z = -13.040 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true },
	{ config_id = 3017, monster_id = 21020301, pos = { x = 0.021, y = 0.029, z = -14.708 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, disableWander = true }
}
...
function action_EVENT_ANY_MONSTER_DIE_3012(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 241008003, 3)
...
function action_EVENT_ANY_MONSTER_DIE_3013(context, evt)
	...
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 241008003, {3017}, 1, 1, 1) then
		return -1
	end
...
function action_EVENT_ANY_MONSTER_LIVE_3022(context, evt)
	...
	if 0 ~= ScriptLib.ActiveChallenge(context, 1, 1, 241008003, 6, 0, 0) then
...
function action_EVENT_CHALLENGE_SUCCESS_3023(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 1, 241008001) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_groupVariable_by_group")
	  return -1
	end
	
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 241008002, suite = 1 }) then
...
function action_EVENT_CHALLENGE_FAIL_3024(context, evt)
		...
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 241008003, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function condition_EVENT_MONSTER_TIDE_DIE_3025(context, evt)
	...
	-- 判断变量"stage"为1
	if ScriptLib.GetGroupVariableValueByGroup(context, "stage", 241008001) ~= 1 then
			return false
	end
