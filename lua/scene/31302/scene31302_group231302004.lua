...
-- 怪物
monsters = {
	{ config_id = 4002, monster_id = 21020201, pos = { x = -2.405, y = 0.034, z = 9.744 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1 },
	...
	{ config_id = 4021, monster_id = 21020202, pos = { x = 2.624, y = 0.034, z = -9.935 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1, affix = { 6105 } },
...
function action_EVENT_ANY_MONSTER_LIVE_4028(context, evt)
	...
	-- 从231302001的变量TPL_TIME中取出对应值并开启挑战
	  local tpl_time = ScriptLib.GetGroupVariableValueByGroup(context, "TPL_TIME", 231302001)
	  ...
	  if 0 ~= ScriptLib.ActiveChallenge(context, 2, 198, tpl_time, 231302004, 5, 0) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : active_challenge_by_remainTime")
	    return -1
	  end
...
function condition_EVENT_ANY_MONSTER_DIE_4029(context, evt)
	...
	if ScriptLib.GetGroupMonsterCountByGroupId(context, 231302004) ~= 0 then
		return false
	end
...
function action_EVENT_ANY_MONSTER_DIE_4029(context, evt)
	-- 添加suite3的新内容
	    ScriptLib.AddExtraGroupSuite(context, 231302004, 3)
...
function action_EVENT_CHALLENGE_SUCCESS_4032(context, evt)
	...
	if 0 ~= ScriptLib.SetGroupVariableValueByGroup(context, "stage", 2, 231302001) then
...
function action_EVENT_CHALLENGE_FAIL_4033(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 231302004, suite = 1 }) then
