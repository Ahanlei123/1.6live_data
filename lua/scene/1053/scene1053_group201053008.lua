...
gadgets = {
	...
	{ config_id = 8018, gadget_id = 70900201, pos = { x = -44.688, y = 60.662, z = 162.794 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
regions = {
	...
	{ config_id = 8017, shape = RegionShape.SPHERE, radius = 5, pos = { x = -81.525, y = 64.586, z = 145.965 } }
}
...
suites = {
	{
		...
		gadgets = { 8016 },
		regions = { 8009, 8017 },
		triggers = { "ENTER_REGION_8009", "ENTER_REGION_8017" },
		rand_weight = 100
	},
	{
		-- suite_id = 2,
...
function action_EVENT_ANY_GADGET_DIE_8005(context, evt)
	-- 延迟3秒后,向groupId为：201053008的对象,请求一次调用,并将string参数："time1" 传递过去
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 201053008, "time1", 3) then
...
function action_EVENT_ANY_GADGET_DIE_8006(context, evt)
	-- 延迟3秒后,向groupId为：201053008的对象,请求一次调用,并将string参数："time2" 传递过去
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 201053008, "time2", 3) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : create_timerevent_by_group")
	  return -1
	end
...
function action_EVENT_ANY_GADGET_DIE_8007(context, evt)
	-- 延迟3秒后,向groupId为：201053008的对象,请求一次调用,并将string参数："time3" 传递过去
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 201053008, "time3", 3) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : create_timerevent_by_group")
	  return -1
	end
...
function action_EVENT_ANY_GADGET_DIE_8008(context, evt)
	-- 延迟3秒后,向groupId为：201053008的对象,请求一次调用,并将string参数："time4" 传递过去
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 201053008, "time4", 3) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : create_timerevent_by_group")
	  return -1
	end
...
function action_EVENT_ENTER_REGION_8009(context, evt)
	...
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 201053009, 2)
