...
-- 区域
regions = {
	{ config_id = 16005, shape = RegionShape.SPHERE, radius = 8, pos = { x = -66.750, y = 126.465, z = 74.812 } }
}

-- 触发器
triggers = {
	{ config_id = 1016003, name = "QUEST_FINISH_16003", event = EventType.EVENT_QUEST_FINISH, source = "7180004", condition = "condition_EVENT_QUEST_FINISH_16003", action = "action_EVENT_QUEST_FINISH_16003" },
...
function condition_EVENT_QUEST_FINISH_16003(context, evt)
	--检查ID为7180004的任务的完成状态是否为1（1=完成，0=失败）
	--此事件需要配合Quest表使用，在Quest表里的完成执行中配置“通知group脚本”，则该任务完成后服务端会向对应的group发送通知，参数1填写场景ID，参数2填写group ID（如果不填则会通知所有group）
	
	--检查任务ID
	if 7180004 ~= evt.param1 then
		return false
	end
...
function action_EVENT_QUEST_FINISH_16003(context, evt)
	-- 延迟1秒后,向groupId为：201053016的对象,请求一次调用,并将string参数："outtime" 传递过去
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 201053016, "outtime", 1) then
...
function action_EVENT_TIMER_EVENT_16004(context, evt)
	...
	if 0 ~= ScriptLib.AddQuestProgress(context, "7180005") then
...
function action_EVENT_ENTER_REGION_16005(context, evt)
	-- 通知任务系统完成条件类型"LUA通知"，复杂参数为quest_param的进度+1
	if 0 ~= ScriptLib.AddQuestProgress(context, "7180004") then
		ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : add_quest_progress")
	  return -1
	end
