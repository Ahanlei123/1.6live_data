...
-- 触发器
triggers = {
	{ config_id = 1010005, name = "VARIABLE_CHANGE_10005", event = EventType.EVENT_VARIABLE_CHANGE, source = "", condition = "condition_EVENT_VARIABLE_CHANGE_10005", action = "action_EVENT_VARIABLE_CHANGE_10005" },
...
-- 变量
variables = {
	{ name = "unlockOperator", value = 0, no_refresh = false }
}
...
suites = {
	{
		...
		gadgets = { 10010 },
		regions = { },
		triggers = { "VARIABLE_CHANGE_10005", "SELECT_OPTION_10007" },
		rand_weight = 100
	},
	{
		-- suite_id = 2,
...
function condition_EVENT_VARIABLE_CHANGE_10005(context, evt)
	...
	-- 判断变量"unlockOperator"为1
	if ScriptLib.GetGroupVariableValue(context, "unlockOperator") ~= 1 then
			return false
	end
...
function action_EVENT_VARIABLE_CHANGE_10005(context, evt)
	...
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 201053010, 10010, {2}) then
...
function action_EVENT_SELECT_OPTION_10007(context, evt)
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 201053010, 2)
	
	-- 添加suite2的新内容
	    ScriptLib.AddExtraGroupSuite(context, 201053013, 2)
	...
	-- 删除指定group： 201053010 ；指定config：10010；物件身上指定option：2；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 201053010, 10010, 2) then
