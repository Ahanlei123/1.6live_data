...
-- 装置
gadgets = {
	{ config_id = 5001, gadget_id = 70360010, pos = { x = 0.079, y = 0.100, z = -0.007 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 }
}
...
function action_EVENT_GADGET_CREATE_5002(context, evt)
	-- 设置操作台选项
	if 0 ~= ScriptLib.SetWorktopOptionsByGroupId(context, 241017005, 5001, {177}) then
...
function condition_EVENT_SELECT_OPTION_5003(context, evt)
	...
	-- 判断变量"stage"为0
	if ScriptLib.GetGroupVariableValueByGroup(context, "stage", 241017006) ~= 0 then
			return false
	end
...
function action_EVENT_SELECT_OPTION_5003(context, evt)
	-- 创建编号为1（该怪物潮的识别id)的怪物潮，创建怪物总数为20，场上怪物最少7只，最多7只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 1, 241017004, {4001,4002,4003,4004,4005,4006,4007,4008,4009,4010,4011,4012,4013,4014,4015,4016,4017,4018,4019,4020}, 20, 7, 7) then
		return -1
	end
	
	-- 删除指定group： 241017005 ；指定config：5001；物件身上指定option：177；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 241017005, 5001, 177) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
	
	-- 改变指定group组241017005中， configid为5001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 241017005, 5001, GadgetState.GearStop) then
...
function action_EVENT_SELECT_OPTION_5004(context, evt)
	-- 创建编号为2（该怪物潮的识别id)的怪物潮，创建怪物总数为20，场上怪物最少7只，最多7只
	if 0 ~= ScriptLib.AutoMonsterTide(context, 2, 241017004, {4001,4002,4003,4004,4005,4006,4007,4008,4009,4010,4011,4012,4013,4014,4015,4016,4017,4018,4019,4020}, 20, 7, 7) then
		return -1
	end
