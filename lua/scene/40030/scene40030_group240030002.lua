...
monsters = {
	...
	{ config_id = 2003, monster_id = 21010701, pos = { x = 527.037, y = -1.983, z = 234.897 }, rot = { x = 0.000, y = 120.000, z = 0.000 }, level = 1, affix = { 1007, 1008 } },
	...
	{ config_id = 2009, monster_id = 21010701, pos = { x = 530.294, y = -1.983, z = 245.781 }, rot = { x = 0.000, y = 180.000, z = 0.000 }, level = 1, affix = { 1007, 1008 } },
	...
	{ config_id = 2016, monster_id = 21010701, pos = { x = 526.504, y = -1.984, z = 242.149 }, rot = { x = 0.000, y = 150.000, z = 0.000 }, level = 1, affix = { 1007, 1008 } },
...
triggers = {
	...
	{ config_id = 1002022, name = "CHALLENGE_FAIL_2022", event = EventType.EVENT_CHALLENGE_FAIL, source = "110107", condition = "", action = "action_EVENT_CHALLENGE_FAIL_2022", trigger_count = 0 },
	{ config_id = 1002023, name = "CHALLENGE_SUCCESS_2023", event = EventType.EVENT_CHALLENGE_SUCCESS, source = "110107", condition = "", action = "action_EVENT_CHALLENGE_SUCCESS_2023", trigger_count = 0 }
}
...
function action_EVENT_ANY_MONSTER_LIVE_2021(context, evt)
	ScriptLib.CreateFatherChallenge(context, 110107, 110107, 90, {success = 2, fail = 1, fail_on_wipe=true})
	ScriptLib.AttachChildChallenge(context, 110107, 110108, 110108, {24000,0,0,0},{},{success=1,fail=1})
	ScriptLib.AttachChildChallenge(context, 110107, 110109, 110109, {7,20,0,0},{},{success=1,fail=1})
	ScriptLib.StartFatherChallenge(context, 110107)
	return 0
end
...
function action_EVENT_CHALLENGE_FAIL_2022(context, evt)
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240030002, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
	
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240030001, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
...
function action_EVENT_CHALLENGE_SUCCESS_2023(context, evt)
	-- 改变指定group组240030001中， configid为1001的gadget的state
	if 0 ~= ScriptLib.SetGroupGadgetStateByConfigId(context, 240030001, 1001, GadgetState.GearStart) then
	      ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : set_gadget_state_by_GroupId_ConfigId")
			return -1
		end 
	
		-- 重新生成指定group，指定suite
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 240030002, suite = 3 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
			return -1
		end
