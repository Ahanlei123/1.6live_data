...
-- 装置
gadgets = {
	{ config_id = 5004, gadget_id = 70360159, pos = { x = 32.272, y = 69.000, z = -45.694 }, rot = { x = 0.000, y = 151.239, z = 0.000 }, level = 1, mark_flag = 2 },
	...
	{ config_id = 5017, gadget_id = 70360161, pos = { x = 98.200, y = 40.000, z = -4.100 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
	{ config_id = 5018, gadget_id = 70360160, pos = { x = -3.900, y = 40.001, z = -80.600 }, rot = { x = 0.000, y = 0.000, z = 0.000 }, level = 1 },
...
triggers = {
	...
	{ config_id = 1005024, name = "VARIABLE_CHANGE_5024", event = EventType.EVENT_VARIABLE_CHANGE, source = "success", condition = "condition_EVENT_VARIABLE_CHANGE_5024", action = "action_EVENT_VARIABLE_CHANGE_5024", tag = "903" },
	{ config_id = 1005025, name = "GADGET_STATE_CHANGE_5025", event = EventType.EVENT_GADGET_STATE_CHANGE, source = "", condition = "condition_EVENT_GADGET_STATE_CHANGE_5025", action = "", tag = "902" },
...
suites = {
	{
		...
		regions = { 5010, 5014 },
		triggers = { "CHALLENGE_FAIL_5001", "TIME_AXIS_PASS_5002", "VARIABLE_CHANGE_5003", "SELECT_OPTION_5006", "TIMER_EVENT_5007", "ENTER_REGION_5010", "GADGET_CREATE_5011", "SELECT_OPTION_5012", "ENTER_REGION_5014", "VARIABLE_CHANGE_5015", "VARIABLE_CHANGE_5020", "VARIABLE_CHANGE_5021", "VARIABLE_CHANGE_5022", "VARIABLE_CHANGE_5023", "VARIABLE_CHANGE_5024", "GADGET_STATE_CHANGE_5025", "CHALLENGE_FAIL_5026" },
		rand_weight = 100
	},
	{
		-- suite_id = 2,
...
function action_EVENT_CHALLENGE_FAIL_5001(context, evt)
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243006004, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243006001, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243006008, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243006009, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243006010, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243006011, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243006016, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
	...
	-- 重新生成指定group，指定suite
	if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243006017, suite = 1 }) then
	    ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : refresh_group_to_suite")
		return -1
	end
...
function action_EVENT_TIME_AXIS_PASS_5002(context, evt)
	-- 重新生成指定group，默认为独立随机
		if 0 ~= ScriptLib.RefreshGroup(context, { group_id = 243006012 }) then
...
function action_EVENT_VARIABLE_CHANGE_5003(context, evt)
	-- 删除指定group： 243006005 ；指定config：5013；物件身上指定option：58；
...
function action_EVENT_TIMER_EVENT_5007(context, evt)
		-- 卸载指定gadget
		if 0 ~= ScriptLib.RemoveEntityByConfigId(context, 243006005, EntityType.GADGET, 5016 ) then
...
function action_EVENT_SELECT_OPTION_5012(context, evt)
	...
	-- 延迟2秒后,向groupId为：243006005的对象,请求一次调用,并将string参数："floor" 传递过去
	if 0 ~= ScriptLib.CreateGroupTimerEvent(context, 243006005, "floor", 2) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : create_timerevent_by_group")
	  return -1
	end
	
	-- 删除指定group： 243006005 ；指定config：5013；物件身上指定option：7；
	if 0 ~= ScriptLib.DelWorktopOptionByGroupId(context, 243006005, 5013, 59) then
	  ScriptLib.PrintContextLog(context, "@@ LUA_WARNING : del_work_options_by_group_configId")
		return -1
	end
...
function action_EVENT_ENTER_REGION_5014(context, evt)
	...
	ScriptLib.StartGallery(context, 7012)
	
	-- 使灭队event失效
	if 0 ~= ScriptLib.ChangeGroupVariableValueByGroup(context, "AvatarDie", 1, 243006014) then
