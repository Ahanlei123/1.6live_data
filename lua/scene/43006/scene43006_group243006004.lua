...
-- 怪物
monsters = {
	{ config_id = 4007, monster_id = 24010301, pos = { x = 31.623, y = -19.559, z = -40.868 }, rot = { x = 0.000, y = 168.261, z = 0.000 }, level = 11, affix = { 1008 }, pose_id = 100 }
}
...
function action_EVENT_SELECT_OPTION_4003(context, evt)
	...
	-- 删除指定group： 243006004 ；指定config：4001；物件身上指定option：7；
...
function condition_EVENT_VARIABLE_CHANGE_4004(context, evt)
	...
	-- 判断变量"key1"为1
	if ScriptLib.GetGroupVariableValue(context, "key1") ~= 1 then
			return false
	end
	
	-- 判断变量"success"为0
	if ScriptLib.GetGroupVariableValueByGroup(context, "success", 243006005) ~= 0 then
			return false
	end
...
function action_EVENT_CHALLENGE_FAIL_4006(context, evt)
	-- 改变指定group组243006013中， configid为4001的gadget的state
...
function action_EVENT_DUNGEON_SETTLE_4009(context, evt)
	ScriptLib.StopGallery(context, 7012, false)
